...
setwd(output_path) 
load(file = "afl_tables_raw.Rdata")     # list - "tables"
load(file="afl_tables.Rdata")
...
teams = as_tibble(unique(afl_tables$Team))
...
afl_tables_all = left_join(afl_tables, teams_all, by = c("Team" = "previous_name"))

# Add additional information of previous season's finishing position
afl_tables = afl_tables_all %>%
...
afl_all_time_ladder = group_by(afl_tables, current_name) %>%
  summarise(count = n(),
            Total_Pld = sum(Pld),
            Total_W = sum(W),
            Total_L = sum(L),
...
premiers = filter(afl_tables, premiers == 1)
premiers_by_Pos = group_by(premiers, Pos) %>%
  summarise(count = n())

# totals by season
season_totals = group_by(afl_tables, season) %>%
  summarise(count = n(),
            Total_Pld = sum(Pld),
            Total_W = sum(W),
            Total_L = sum(L),
...
title_race_totals = group_by(afl_tables, season, yr_end) %>%
  summarise(count = n(),
...
# totals by club
club_records = group_by(afl_tables, current_name) %>%
...
highest_PF_team = club_records %>%
  left_join(afl_tables, by = c("current_name" = "current_name",
                               "highest_PF" = "PF")) %>%
...
most_winning_margin_season = title_race_totals %>%
  ...
  left_join(afl_tables, by = c("season" = "season")) %>%
  filter(Pos == 1) %>%
...
# Longest streaks
longest_streaks_finals = arrange(afl_tables, desc(streak_finals)) %>%
  select(season, Team, streak_finals)
head(longest_streaks_finals, 5)

longest_streaks_missed_finals = arrange(afl_tables, desc(streak_missed_finals)) %>%
  select(season, Team, streak_missed_finals)
head(longest_streaks_missed_finals, 5)


# no. of teams in finals
finals_teams = afl_tables %>% 
...
teams_unique = unique(afl_tables$abbrev)


# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# checks on data for consistency
error_check_pts = afl_tables %>% 
  filter(!Pts == (pts_per_win * (W + B) + pts_per_draw * D))    

error_check_pld = afl_tables %>%
  filter(!Pld == (W + D + L))
...
error_check_perc = afl_tables %>%
  filter(!(abs(perc_check) < 0.1))

error_check_pos = group_by(afl_tables, season) %>%
  summarise(count = n(),
            sum_pos = sum(Pos)) %>%
...
setwd(output_path)
save(tables, file = "afl_tables_raw.Rdata")
save(afl_tables, file = "afl_tables.Rdata")
write.csv(afl_tables, file = "afl_tables_full.csv")
...
# To do:
# add to records: longest streaks for new variables cum_premiers, cum_runners_up, cum_minor_premiers, cum_grand_finals
